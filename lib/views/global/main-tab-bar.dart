  ...
  Widget build(BuildContext context) {
    return CupertinoTabScaffold(
      ...
      tabBuilder: (context, index) {
        if(index == 0) {
          return CupertinoTabView(navigatorKey: firstTabNavKey, builder: (BuildContext context) => Home(),);
        }
        else if(index == 1) {
          return CupertinoTabView(navigatorKey: secondTabNavKey, builder: (BuildContext context) => SchedulePage(),);
        }
        else {
          return CupertinoTabView(navigatorKey: thirdTabNavKey, builder: (BuildContext context) => ProfilePage(),);
        }
      },
    );
  }
